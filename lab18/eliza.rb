...
class Shrink
  ...
  def generateResponse(blather)
    ...
    blather.gsub!(/\bwell\b.*/,"")
    blather.gsub!(/\bperhaps\b.*/,"")

    # \b = stand alone words and not part of others
    neverAlways=/.*\b(always|never).*/
    blather.sub!(neverAlways,'CAN YOU BE MORE SPECIFIC?')

    qu = /\b(ARE I)\b(.*)/
    blather.sub!(qu, 'IS IT IMPORTANT IF I AM\1?')
