...
> myFoldl :: (a -> b -> a) -> a -> [b] -> a
> myFoldl func accum (x:xs) = myFoldl func (func accum x) xs


Next, define a function to reverse a list using foldl.

> myReverse :: [a] -> [a]
> myReverse (x:xs) = myFoldl (\acc item -> item:acc) [] x

append :: String -> [String] -> [String]
append a [] = [a]
append a (x:xs) = x : append a xs 

myReverse (x:xs) = myFoldl append x xs


Now define your own version of foldr, named myFoldr

> myFoldr :: (a -> b -> b) -> b -> [a] -> b
> myFoldr func accum (x:xs) = func x (myFoldr func accum xs)
...
> sumAbs  lst = foldr (+) 0 (map abs lst)
